...
int main()
{
    ...
    if (fs::exists("__pycache__"))
    {
        ...
        for (auto &&entry : entries)
        {
            ...
            if (std::regex_search(file.filename().string(), std::regex("^gui")))
            {
                fs::rename(file, "pyBytecode/gui.pyc");
            }
            else if (std::regex_search(file.filename().string(), std::regex("^main")))
            {
                fs::rename(file, "pyBytecode/main.pyc");
            }
        }
    }
